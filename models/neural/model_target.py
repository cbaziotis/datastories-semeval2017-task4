...
if TASK == "BD":
    loader = Task4Loader(word_indices, text_lengths=(target_max_length, text_max_length), subtask=TASK,
                         filter_classes={"positive", "negative"}, y_one_hot=False)
...
nn_model = target_RNN(embeddings,
                      tweet_max_length=text_max_length, aspect_max_length=target_max_length,
                      noise=0.2,
                      activity_l2=0.001,
                      drop_text_rnn_U=0.2,
                      drop_text_input=0.3,
...
if TASK == "BD":
    ...
    metrics = {
        ...
        "recall": (lambda y_test, y_pred: recall_score(y_test, y_pred, average='macro')),
        "precision": (lambda y_test, y_pred: precision_score(y_test, y_pred, average='macro'))
    }
else:
    ...
    metrics = {
        "macro_mae": macro_mae,
        "micro_mae": (lambda y_test, y_pred: mean_absolute_error(y_test, y_pred)),
    }
...
if TASK == "BD":
    plotting = PlottingCallback(grid_ranges=(0.75, 1), height=4, benchmarks={"ρ": 0.797, "α": 0.87})
    checkpointer = ModelCheckpoint(filepath=best_model(), monitor='val.recall', mode="max", verbose=1, save_best_only=True)
else:
    plotting = PlottingCallback(grid_ranges=(0.4, 1.), height=4, benchmarks={"MAE_M": 0.719, "MAE_m": 0.58})
...
if TASK == "BD":
    ...
    class_weights = get_class_weights2(onehot_to_categories(training[1]), smooth_factor=0.1)
...
history = nn_model.fit(training[0], training[1],
                       ...
                       nb_epoch=50, batch_size=64, class_weight=class_weights, callbacks=_callbacks)
